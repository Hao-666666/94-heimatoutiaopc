<template>
  <div class="login">
    ...
    <el-card class='login-card'>
      ...
      <!-- 放置表单  绑定model属性 绑定rules属性（表单验证规则）ref给el-form一个属性-->
      <el-form ref="loginForm" :model="loginForm" :rules="loginRules" style="margin-top:20px">
        ...
        <el-form-item prop="checked">
          <!-- 复选框 -->
          <el-checkbox style="color:white" v-model="loginForm.checked">我已阅读并同意用户协议和隐私条款</el-checkbox>
        </el-form-item>
        <!-- 按钮 -->
        <el-form-item>
    ...
    login () {
      ...
      this.$refs.loginForm.validate().then(() => {
        ...
        this.$axios({
          ...
          // 跳转到主页
          this.$router.push('/') // push 和 router-link类似 to属性 可以直接是字符串 也可以是对象
        }).catch(() => {
          // 提示消息
          // 第一种用法
...
.login {
  ...
  .login-card {
    background: rgba(0, 0, 0, 0.8); //登录窗口毛玻璃效果
    z-index: 2;
    width:440px;
    height: 340px;
    .title {
